...
export default function ListeningListScreen({ navigation }) {
  ...
  const fetchTests = useCallback(async (pageNum = 1, isLoadMore = false) => {
    try {
      ...
      if (pageNum === 1) {
        ...
        const testsRes = await getListeningTests(apiParams);
        testItems = testsRes.data?.data || [];
        totalCount = testsRes.data?.total || 0;
      }
      ...
      console.error('Lỗi load đề nghe:', e.message);
      if (!isLoadMore) {
        setTests([]);
      }
    } finally {
  ...
  const loadCacheAndFetch = useCallback(async () => {
    const cached = await getCache(`listening_tests_${activeFilter}`);
    if (cached) {
  ...
  const renderFooter = () => {
    ...
    return (
      <View style={styles.footerLoader}>
        <ActivityIndicator size="small" color={isDarkMode ? '#90CAF9' : '#1565C0'} />
      </View>
    );
  };
  ...
  const renderItem = ({ item }) => {
    const progressWidth = item.totalQuestions
      ...
      : '0%';
    const progressColor = isDarkMode ? '#90CAF9' : '#1565C0';
    const progressBg = isDarkMode ? 'rgba(144, 202, 249, 0.15)' : '#E3F2FD';
      ...
      >
        <View style={styles.cardTop}>
          <View style={styles.cardLeft}>
            <View style={[styles.cardIcon, { backgroundColor: isDarkMode ? 'rgba(144, 202, 249, 0.15)' : '#E3F2FD' }]}>
              <Ionicons name="headset" size={20} color={isDarkMode ? '#90CAF9' : '#1565C0'} />
            </View>
            <View style={styles.cardInfo}>
              ...
              {item.status === 'inProgress' ? (
                <Text style={[styles.cardDraftText, { color: isDarkMode ? '#90CAF9' : '#1565C0' }]}>Đang làm dở</Text>
              ) : null}
              {item.status === 'done' ? (
                <View style={styles.scoreWrap}>
                  ...
                  <Text style={[styles.scoreText, { color: isDarkMode ? '#90CAF9' : '#1565C0' }]}>
          ...
          <View style={styles.cardRight}>
            {item.status === 'done' ? (
              ...
              <Text style={[styles.inProgressText, { color: isDarkMode ? '#90CAF9' : '#1565C0' }]}>Đang làm</Text>
            ) : (
        ...
        <TouchableOpacity
          style={[styles.actionBtn, { borderColor: isDarkMode ? '#90CAF9' : '#1565C0' }]}
          onPress={() => openListeningTest(item)}
        >
          <Text style={[styles.actionBtnText, { color: isDarkMode ? '#90CAF9' : '#1565C0' }]}>{getActionLabel(item)}</Text>
        </TouchableOpacity>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.background }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>Luyện Nghe</Text>
      ...
      >
        {FILTERS.map((filter) => {
          ...
          const btnBg = isActive ? (isDarkMode ? '#90CAF9' : '#1565C0') : theme.card;
          const btnBorder = isActive ? (isDarkMode ? '#90CAF9' : '#1565C0') : theme.border;
      ...
      <View style={styles.listHeader}>
        <View style={styles.listHeaderLeft}>
          ...
          <View style={[styles.countBadge, { backgroundColor: isDarkMode ? '#90CAF9' : '#1565C0' }]}>
            <Text style={[styles.countBadgeText, { color: isDarkMode ? '#121212' : '#fff' }]}>{total}</Text>
          </View>
        </View>
        <Text style={[styles.seeAll, { color: isDarkMode ? '#90CAF9' : '#1565C0' }]}>Xem tất cả</Text>
      </View>
        ...
        <FlatList
          ...
          refreshControl={
            <RefreshControl
              ...
              colors={[isDarkMode ? '#90CAF9' : '#1565C0']}
