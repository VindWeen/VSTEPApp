...
export default function SpeakingListScreen({ navigation }) {
  ...
  const fetchTests = useCallback(async (pageNum = 1, isLoadMore = false) => {
    try {
      ...
      if (pageNum === 1) {
        ...
        await setCache(`speaking_tests_${activeFilter}`, { data: processedItems, total: totalCount });
      } else {
        setTests(processedItems);
        setTotal(totalCount);
        setPage(pageNum);
      ...
      console.error('Lỗi load đề nói:', e.message);
      if (!isLoadMore) {
        setTests([]);
      }
    } finally {
  ...
  const loadCacheAndFetch = useCallback(async () => {
    const cached = await getCache(`speaking_tests_${activeFilter}`);
    if (cached) {
  ...
  const renderFooter = () => {
    ...
    return (
      <View style={styles.footerLoader}>
        <ActivityIndicator size="small" color={isDarkMode ? '#E040FB' : '#6A1B9A'} />
      </View>
    );
  };
  ...
  const renderItem = ({ item }) => {
    const progressBg = isDarkMode ? 'rgba(106, 27, 154, 0.15)' : '#F3E5F5';
    const progressColor = isDarkMode ? '#E040FB' : '#6A1B9A';
    const cardIconBg = isDarkMode ? 'rgba(106, 27, 154, 0.15)' : '#F3E5F5';
    const primaryColor = isDarkMode ? '#E040FB' : '#6A1B9A';
        ...
        <View style={styles.cardTop}>
          <View style={styles.cardLeft}>
            ...
            <View style={styles.cardInfo}>
              ...
              <Text style={[styles.cardParts, { color: primaryColor }]}>
                {item.tasks
  ...
  return (
    <SafeAreaView style={[styles.safeArea, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: theme.background }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.text }]}>Luyện Nói</Text>
      ...
      <View style={[styles.stepsRow, { backgroundColor: isDarkMode ? 'rgba(106, 27, 154, 0.15)' : '#F3E5F5' }]}>
        {STEPS.map((step, index) => (
          <React.Fragment key={step.label}>
            <View style={styles.stepItem}>
              <View style={[styles.stepIconBg, { backgroundColor: isDarkMode ? 'rgba(106, 27, 154, 0.25)' : '#E1BEE7' }]}>
                <Ionicons name={step.icon} size={16} color={primaryColor} />
              </View>
              <Text style={[styles.stepLabel, { color: primaryColor }]}>{step.label}</Text>
            </View>
            {index < STEPS.length - 1 ? (
              <Ionicons
                name="chevron-forward"
                size={14}
                color={isDarkMode ? '#555' : '#B0BEC5'}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Ionicons name="mic-off-outline" size={60} color={isDarkMode ? '#444' : '#CFD8DC'} />
